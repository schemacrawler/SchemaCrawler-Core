...
public class ObjectToStringFormatTest {
  ...
  public void nullArgs() {
    assertThat(new ObjectToJsonFormat(null).get(), is("null"));
  }

  @Test
  public void stringArg() {
    // Strings serialize as JSON strings (quoted)
    assertThat(new ObjectToJsonFormat("hello, world").get(), is("\"hello, world\""));
    // toString() delegates to get()
    assertThat(
        new ObjectToJsonFormat("hello, world").get(),
        is(new ObjectToJsonFormat("hello, world").toString()));
  }

  @Test
  public void listArg() {
    final List<String> list = List.of("one", "two", "three");
    assertThat(new ObjectToJsonFormat(list).get(), is("[ \"one\", \"two\", \"three\" ]"));
  }
  ...
  public void mapArg() {
    ...
    map.put("three", 3);
    map.put("two", 2);
    final String result = new ObjectToJsonFormat(map).get();
  ...
  public void objectArg() {
    final String result = new ObjectToJsonFormat(new SomeClass("hello, world", 42)).get();
