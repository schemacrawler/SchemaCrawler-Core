 ...
 *
 * <p>This is analogous to {@link schemacrawler.crawl.WeakAssociationBuilder} for weak associations
 * in the catalog. Use it to add implicit associations discovered via {@link
 * schemacrawler.ermodel.associations.ImplicitAssociationsAnalyzer} into an existing {@link ERModel}.
 */
public final class ImplicitAssociationBuilder {

  private static final Logger LOGGER =
      Logger.getLogger(ImplicitAssociationBuilder.class.getName());

  /**
   * Creates a new builder for adding implicit associations to the given ER model.
   *
   * @param erModel Mutable ER model to enrich; must be a {@link MutableERModel}
  ...
  public static ImplicitAssociationBuilder builder(final ERModel erModel) {
    ...
    if (!(erModel instanceof final MutableERModel mutableERModel)) {
      throw new IllegalArgumentException("ERModel must be a MutableERModel");
    }
