 ...
 * model, analogous to how {@link
 * schemacrawler.tools.loader.catalog.weakassociations.WeakAssociationsCatalogLoader} works for weak
 * associations in the catalog.
 *
...
public final class ImplicitAssociationsERModelLoader
  ...
  public void execute() {
    ...
    try (final TaskRunner taskRunner =
        TaskRunners.getTaskRunner("loadImplicitAssociations", 1)) {
      taskRunner.add(
          new TaskDefinition("retrieveImplicitAssociations", this::loadImplicitAssociations));
      taskRunner.submit();
