...
public final class MetaDataUtility {
  ...
  private static final List<Pattern> SYSTEM_GENERATED_FK_PATTERNS =
      List.of(
          Pattern.compile("INTEG_[0-9]+"), // Firebird
          Pattern.compile("CONSTRAINT_[A-Z0-9]+"), // H2
          Pattern.compile("SYS_FK_[0-9]{5}"), // HSQLDB
          Pattern.compile("SQL[0-9]{15}"), // DB2
          Pattern.compile(".*_ibfk_1"), // MySQL, MariaDB
          Pattern.compile("SYS_C.*"), // Oracle
          // Pattern.compile(".*_fkey"), // PostgreSQL
          Pattern.compile("FK__.*__.*__[A-Z0-9]{8}"), // SQL Server
          Pattern.compile("SCHCRWLR_.*")); // SchemaCrawler
   ...
   */
  public static boolean isSystemGeneratedForeignKeyName(final TableReference fk) {
    if (fk == null || fk.getName().isEmpty()) {
      return false;
    }
    for (final Pattern pattern : SYSTEM_GENERATED_FK_PATTERNS) {
      if (pattern.matcher(fk.getName()).matches()) {
        return true;
      }
    }
    return false;
